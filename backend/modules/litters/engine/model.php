...
class Disabled{
	/**
	 * Checks if a certain parent exists
	 *
	 * @param int $id
	 *
	 * @return bool
...
EOQ;
	...
	/**
	 * Delete a certain parent
	 *
	 * @param int $id
	 */
	public static
	function deleteParent($id){
		BackendModel::getContainer()->get('database')->delete('litters_parents', 'id = ?', (int)$id);
	}

	/**
	 * Insert an parent in the database
	 *
	 * @param array $item
	 *
	 * @return int
	 */
	public static
	function insertParent(array $item){
		$item['created_on'] = BackendModel::getUTCDate();
		$item['edited_on']  = BackendModel::getUTCDate();

		return (int)BackendModel::getContainer()->get('database')->insert('litters_parents', $item);
	}

	/**
	 * Insert an young in the database
	 *
	 * @param array $item
	 *
	 * @return int
	 */
	public static
	function insertYoung(array $item){
		$item['created_on'] = BackendModel::getUTCDate();
		$item['edited_on']  = BackendModel::getUTCDate();

		return (int)BackendModel::getContainer()->get('database')->insert('litters_youngs', $item);
	}
	 ...
	 */
	public static
	function updateYoung(array $item){
		$item['edited_on'] = BackendModel::getUTCDate();

		BackendModel::getContainer()->get('database')->update('litters_youngs', $item, 'id = ?', (int)$item['id']);
	}
	 ...
	 */
	public static
	function updateParent(array $item){
		$item['edited_on'] = BackendModel::getUTCDate();

		BackendModel::getContainer()->get('database')->update('litters_parents', $item, 'id = ?', (int)$item['id']);
	}
	...
	function getAllParents(){
		$result = (array)BackendModel::getContainer()->get('database')->getRecords(self::QRY_DATAGRID_BROWSE_PARENTS, array(BL::getWorkingLanguage()));
	 ...
	 */
	public static
	function getAllYoungs(){
		$result = (array)BackendModel::getContainer()->get('database')->getRecords(self::QRY_DATAGRID_BROWSE_YOUNGS, array(BL::getWorkingLanguage()));

		return $result;
	}

	/**
	 * Checks if a certain young exists
	 *
	 * @param int $id
	 *
	 * @return bool
...
SELECT
	i.*,
	UNIX_TIMESTAMP(i.birth_date) AS birth_date
FROM litters_parents AS i
WHERE i.id = ?;
EOQ;
		return (array)BackendModel::getContainer()->get('database')->getRecord( $req,  array((int)$id));
	}
