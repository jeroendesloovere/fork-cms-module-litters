...
{form:edit}
				...
				<tr>
					...
					<td id="sidebar">
						...
						<div class="box">
							...
							<div class="options">
								<table id="presentation_data">
									<tr>
										<th><label for="url">{$lblPersonalPageUrl|ucfirst}</label></th>
										<th colspan="2"><label for="photo">{$lblPhoto|ucfirst}</label></th>
									</tr>
									<tr>
										<td><p>{$txtPersonalPageUrl} {$txtPersonalPageUrlError}</p></td>
										<td>{option:photo_url}
												<img src="{$photo_url}" width="200" alt="{$item.name}" />{/option:photo_url}
										</td>
	...
	<div class="fullwidthOptions">
		<a href="{$var|geturl:'delete_parent'}&amp;id={$item.id}" data-message-id="confirmDelete" class="askConfirmation button linkButton icon iconDelete">
	...
	<div id="confirmDelete" title="{$lblDelete|ucfirst}?" style="display: none;">
		<p>{$msgConfirmDeleteParent|sprintf:{$item.name}}</p>
	</div>
{/form:edit}
